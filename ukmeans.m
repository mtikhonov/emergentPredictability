function varargout = ukmeans(X, err, k, varargin)
    ...
    function cellout = loopBody(rep,S)
        ...
        switch start
            ...
            case {'plus','kmeans++'}
               ...
               for ii = 2:k                    
                    minDist = min(minDist,distfun(X,err,C(:,ii-1),distance));
        ...
        D = distfun(X, err, C, distance, 0, rep, reps);
        [d, idx] = min(D, [], 2);
        ...
        try % catch empty cluster errors and move on to next rep
            ...
            nonempties = find(m>0);
            D(:,nonempties) = distfun(X, err, C(:,nonempties), distance, iter, rep, reps);
            d = D((idx-1)*n + (1:n)');
        ...
        function converged = batchUpdate()
            ...
            while true
                ...
                D(:,changed) = distfun(X, err, C(:,changed), distance, iter, rep, reps);
                    ...
                    switch emptyact
                        ...
                        case 'singleton'
                            for i = empties
                                ...
                                m(i) = 1;
                                idx(lonely) = i;
                                D(:,i) = distfun(X, err, C(:,i), distance, iter, rep, reps);
                                
                                % Update clusters from which points are taken
                                [C(:,from), m(from)] = gcentroids(X, idx, from, distance);
                                D(:,from) = distfun(X, err, C(:,from), distance, iter, rep, reps);
...
function D = distfun(X, err, C, dist, iter,rep, reps)
%DISTFUN Calculate point to cluster centroid distances.

switch dist
    case 'sqeuclidean'
        % if strcmp(version('-release'), '2023a')
        D = internal.stats.pdist2mex(X,C,'sqe',[],[],[],[])./(err.^2);  
    case {'cosine','correlation'}
        % The points are normalized, centroids are not, so normalize them
        normC = sqrt(sum(C.^2, 1));
        if any(normC < eps(class(normC))) % small relative to unit-length data points
