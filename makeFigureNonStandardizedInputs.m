function makeFigureNonStandardizedInputs
options.complexity_opt = 'entropy';
options.input_frmt = 'raw';
generateClarkKehePareto(options);
plotFigure;
end

function plotFigure
load('figureData/clark_kehe_paretofronts_inputsRaw_complexityAsEntropy.mat')
figure
clf
...
for obs = 1:length(observables)
    ...
    if obs < length(observables)
        ...
        if obs < 2
            ...
            text(lhand.Position(1)+2.1,lhand.Position(2)+0.475,lhand.Position(3),'Richness','FontSize',font_sz-1)
        end
        if obs == 3
            xlabel(ax,'Description Complexity')
        end
